...
class TestTrainingStatus:
    def test_status_no_runs(self, client, auth_headers, project_id):
        resp = client.get(f"/projects/{project_id}/train/status", headers=auth_headers)
    ...
    def test_status_after_configure(self, client, auth_headers, project_id):
        ...
        client.post(f"/projects/{project_id}/train/configure", json={
            "base_model": "TinyLlama/TinyLlama-1.1B-Chat-v1.0",
        }, headers=auth_headers)
...
class TestTrainingHistory:
    def test_empty_history(self, client, auth_headers, project_id):
        resp = client.get(f"/projects/{project_id}/train/history", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
        assert data["runs"] == []
        assert data["total"] == 0
...
class TestTrainingControls:
    def test_start_without_config(self, client, auth_headers, project_id):
        resp = client.post(f"/projects/{project_id}/train/start", headers=auth_headers)
        assert resp.status_code == 400

    def test_pause_without_active(self, client, auth_headers, project_id):
        resp = client.post(f"/projects/{project_id}/train/pause", headers=auth_headers)
        assert resp.status_code == 400

    def test_stop_without_active(self, client, auth_headers, project_id):
        resp = client.post(f"/projects/{project_id}/train/stop", headers=auth_headers)
        assert resp.status_code == 400
